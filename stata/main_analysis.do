/*******************************************************************************
    ...
    Author: Reha Tuncer
    Date: 25.07.2024
    Description: Rerforms the main analysis
*******************************************************************************/
...
//# table 3bis: Twice referring peers and SES bias
qui reg pcent_twice_t1 c.z_gpa c.z_rav c.z_eye i.ses , vce(cluster net_class)
estimates store twice_1
qui reg pcent_single_rav_t1 c.z_gpa c.z_rav i.ses, vce(cluster net_class)
estimates store single_rav
qui reg pcent_single_eye_t1 c.z_gpa c.z_eye i.ses, vce(cluster net_class)
estimates store single_eye
esttab twice_1 single*,  b(%12.3f) se(%12.3f) r2 nobaselevels label mtitles star(* 0.10 ** 0.05 *** 0.01)

qui reghdfe pcent_twice_t1  c.z_gpa c.z_rav c.z_eye i.ses, absorb(net_class) vce(cluster net_class)
estimates store fe3c
qui reghdfe pcent_single_rav_t1  c.z_gpa c.z_rav i.ses, absorb(net_class) vce(cluster net_class)
estimates store fe3r
qui reghdfe pcent_single_eye_t1 c.z_gpa c.z_eye i.ses, absorb(net_class) vce(cluster net_class)
estimates store fe3e
**# Bookmark #2
esttab  twice_1 fe3c single_rav fe3r single_eye fe3e,  b(%12.3f) se(%12.3f) r2 nobaselevels label mtitles star(* 0.10 ** 0.05 *** 0.01)
...
//# table 5: effect of treatment on referral types
preserve
expand 2
bysort net_id: gen treat_dummy = _n
label define tlabel 1 "Baseline" 2 "Quota"
