...
% Sensitivity analysis incrementally adding academic performance measures is in Appendix Table \ref{tab:regression-results-1-appendix1-cognitive} and Table \ref{tab:regression-results-1-appendix1-social}.
  ...
  In the following empirical specification, we document whether Baseline and Quota treatments cause referral shares to change, controlling for GPA and classroom SES composition. Our dependent variable is the fraction of referrals received by total number of referrals as specified before. We consider referrals for both skills combined and now decompose it by treatment to compare referral shares depending on the referrer's treatment. We estimate referral percentage $y_{i}$: 
  \begin{equation}\label{eq2}
    y_{i} = \beta_0 + \beta_1{GPA}_{i} + \beta_2{SES}_{i} + \beta_3{ClassroomSES}_{i}  + \mathbf{X}_{i}\boldsymbol{\gamma} + \epsilon_{i}    
  \end{equation}
  In addition to GPA and individual controls (age, gender, semester) in $\mathbf{X}_{i}$, we include referral SES and the percentage of low SES classmates as independent variables as specified in our preregistration.\footnote{The relationship between social class and referrals may operate through other channels than socioeconomic status, for example being a first-generation college student, having rural background, etc., which correlate strongly  with low SES. We focus on the effect of social class through SES only.} 
  
  Table \ref{tab:results2} illustrates our findings. Our comparison of interest is estimates across treatments. We obtain the largest estimates for GPA in column (1), documenting the effect of performance-pay incentives in the Baseline treatment without regard for social class. A one standard deviation increase in GPA increases the fraction referrals received from participants in the Baseline condition by 3.8 percentage points on a base rate of 18\%, a 21 percent increase. This estimate is slightly smaller in the Quota condition (column 2): A one standard deviation increase in GPA only increases the fraction of referrals by 17 percent.

  We find no bias against low SES in either treatment, and also when treatments are combined as a consequence. Point estimates for low SES dummy are close to zero in all three columns. The 95\% confidence intervals rule out that low SES causes more than a 1.5 percentage point difference in the fraction of referrals received. Smaller point estimates and substantially lower explained variance in column 2 suggests our model is better at capturing the referral decisions in the Baseline treatment. Further, the Quota treatment causes no meaningful increase in the referrals received by low SES classmates when controlling for GPA and classroom SES composition, contrary to our hypothesis. In the next section we expand on this finding by decomposing referrals further by referrer SES. This will help us identify whether there is social class bias between peers sharing the same social class.  
  \begin{table}[H]
    \centering
    \begin{threeparttable}
    \caption{Percentage of referrals across treatment conditions}\label{tab:results2}
    \begin{tabular*}{0.85\textwidth}{@{\extracolsep{\fill}}l c c c@{}}
    \toprule
    & (1) & (2) & (3) \\
    Dep. Var. & Baseline & Quota & All \\
    \midrule
    GPA & 3.769\tnote{***} & 2.464\tnote{***} & 3.066\tnote{***} \\
    & (0.510) & (0.511) & (0.413) \\
    Low SES & -0.489 & -1.079 & -0.784 \\
    & (0.907) & (1.010) & (0.764) \\
    \% Low SES in class & -0.037 & -0.007 & -0.022 \\
    & (0.040) & (0.041) & (0.033) \\
    Constant & 18.003\tnote{***} & 15.191\tnote{***} & 16.659\tnote{***} \\
    & (3.802) & (3.679) & (2.919) \\
    \midrule
    Individual controls & Yes & Yes & Yes \\
    Observations & 665 & 665 & 665 \\
    R-squared & 0.119 & 0.044 & 0.110 \\
    \bottomrule
    \end{tabular*}
    \begin{tablenotes}[flushleft]
    \footnotesize
    \item[] \textit{Note:} Robust standard errors in parentheses. * $p < 0.1$, ** $p < 0.05$, *** $p < 0.01$. The dependent variable is the percentage of referrals received relative to potential referrals. Column (1) shows results for baseline treatment, column (2) for quota treatment, and column (3) for all referrals. Controls include gender, age and semester. Sample is restricted to participants with complete administrative and experimental data.
    \end{tablenotes}
    \end{threeparttable}
  \end{table}
  ...
  \subsection{Social Class Bias}
  In the following empirical specification, we document social class homophily, controlling classroom SES composition.\footnote{See \citet{mcpherson2001birds} for a discussion on homophily in general, for evidence on social class homophily see \citet{chetty2022social,chetty2022socialb}. For experimental evidence on a different type of homophily (gender) in referrals see \citet{beaman_job_2018}.} Our dependent variable is the fraction of referrals received by total number of referrals as specified before, but now decomposed by the social class of referrers. We begin with the share of referrals coming from high SES, then present the equivalent analysis for low SES. We estimate referral percentage $y_{i}$ the same way in \ref{eq2}.

  In Table \ref{tab:homophily-rob1}, we examine how the share of referrals received differ by referrer's SES. Column 1 shows referrals from high SES peers: Low SES decreases referrals by 1.89 percentage points, and is marginally significant ($p < 0.1$). The size of the estimate is 75 percent of the effect of a one standard deviation increase GPA, which is substantial. On a base rate of 15.8\%, low SES classmates are referred 12 percent less by their high SES peers. In contrast, the estimate for the referral share from low SES peers (column 2) does not significantly differ by low SES, suggesting the homophily exists only between high SES referring other high SES peers. Lower explained variance for high SES referrers suggest our model captures the low SES referral decisions better. The overall effect (column 3) shows low SES students receive 0.78 percentage points fewer referrals. Though this difference is not statistically significant, it is driven by high SES referrers. Academic performance, measured by GPA, remains a strong predictor across all specifications, with slightly larger effects for referrals from low SES peers (3.47 versus 2.49 percentage points). Classroom SES composition has no significant effect on the referral share in any specification.  

  \begin{table}[H]
    \centering
    \begin{threeparttable}
    \caption{Percentage of referrals by referrer's socioeconomic background}\label{tab:homophily-rob1}
    \begin{tabular*}{0.85\textwidth}{@{\extracolsep{\fill}}l c c c@{}}
    \toprule
    & (1) & (2) & (3) \\
    Dep. Var. & High SES & Low SES & All \\
    \midrule
    GPA & 2.493\tnote{***} & 3.470\tnote{***} & 3.066\tnote{***} \\
    & (0.555) & (0.506) & (0.413) \\
    Low SES & -1.895\tnote{*} & 0.245 & -0.784 \\
    & (0.996) & (0.937) & (0.764) \\
    \% Low SES & -0.034 & -0.032 & -0.022 \\
    & (0.041) & (0.046) & (0.033) \\
    Constant & 15.797\tnote{***} & 17.580\tnote{***} & 16.659\tnote{***} \\
    & (3.672) & (3.853) & (2.919) \\
    \midrule
    Individual Controls & Yes & Yes & Yes \\
    Observations & 665 & 665 & 665 \\
    R-squared & 0.056 & 0.091 & 0.110 \\
    \bottomrule
    \end{tabular*}
    \begin{tablenotes}[flushleft]
    \footnotesize
    \item[] \textit{Note:} Robust standard errors in parentheses. * $p < 0.1$, ** $p < 0.05$, *** $p < 0.01$. The dependent variable is the percentage of referrals received relative to potential referrals from high SES (1), low SES (2), and all classmates (3). Individual controls include gender, age and semester. Sample is restricted to participants with complete administrative and experimental data.
    \end{tablenotes}
    \end{threeparttable}
  \end{table}
  ...
  Taken together, these results reveal an asymmetric homophily pattern: While high SES referrers show significant SES homophily that can be overcome by academic performance, low SES referrers appear to base their decisions primarily on performance regardless of social class. This suggests that the barriers to cross-class referrals originate from high-SES students, but are overcome through academic achievement in our sample.
...
\begin{table}
  ...
  \begin{table}
    \centering
    \begin{threeparttable}
    \caption{Comparison table for skills and GPA across the sample versus referrals}\label{tab:ref_comparison}
    \begin{tabular*}{0.85\textwidth}{@{\extracolsep{\fill}}l c c c@{}}
    \toprule
    & Sample & Referrals & $\Delta$ in SD\\
    \midrule
    Cognitive skill & 0.023 & 0.112 & 0.089 \\
    & (0.982) & (1.009) & \\
    GPA & 0.044 & 0.251 & 0.207 \\
    & (1.003) & (1.021) & \\
    \midrule
    Social skill & 0.015 & 0.042 & 0.027 \\
    & (0.985) & (1.009) & \\
    GPA & 0.044 & 0.212 & 0.168 \\
    & (1.003) & (1.004) & \\
    \bottomrule
    \end{tabular*}
    \begin{tablenotes}[flushleft]
    \footnotesize
    \item[] \textit{Note:} Table compares standardized skill and GPA scores of the participants (N = 691) and their referrals (N = 1759). Positive differences indicate higher scores for referrals. The larger differences in GPA compared to skills suggest that overall academic performance is a more salient criterion for referrals.
    \end{tablenotes}
    \end{threeparttable}
   \end{table} 
  ...
  \begin{table}
    \centering
    \begin{threeparttable}
    \caption{Comparison of Skills and GPA: Self-Referrals vs Others}\label{tab:self_ref}
    \begin{tabular*}{0.85\textwidth}{@{\extracolsep{\fill}}l c c c@{}}
    \toprule
    & No self-referral & Any self-referral & $\Delta$ in SD \\
    \midrule
    GPA & 0.035 & -0.121 & 0.156 \\
    & (1.009) & (0.964) & \\
    N & 576 & 226 & \\
    \midrule
    Cognitive Skills & 0.076 & -0.143 & 0.219 \\
    & (0.980) & (1.024) & \\
    N & 465 & 248 & \\
    \midrule
    Social Skills & 0.032 & -0.060 & 0.092 \\
    & (1.013) & (0.975) & \\
    N & 465 & 248 & \\
    \bottomrule
    \end{tabular*}
    \begin{tablenotes}[flushleft]
    \footnotesize
    \item[] \textit{Note:} Table compares standardized scores between participants who self-referred at least once and those who did not. Positive differences indicate higher scores for those who never self-referrered. The results suggest self-referrers have systematically lower scores. This is consistent with the fact that we instructed explicitly against making self-referrals. Standard deviations in parentheses, samples restricted to participants with complete administrative and experimental data.
    \end{tablenotes}
    \end{threeparttable}
  \end{table}
