...
\begin{table}
  \centering
  \begin{threeparttable}
  \caption{Percentage of referrals conditional on performance}\label{tab:regression-results1}
  \begin{tabular*}{0.85\textwidth}{@{\extracolsep{\fill}}l c c c@{}}
  \toprule
  & (1) & (2) & (3) \\
  Dep. Var. & Cognitive & Social & All \\
  \midrule
  GPA & 3.211\tnote{***} & 2.723\tnote{***} & 2.967\tnote{***} \\
  & (0.469) & (0.489) & (0.441) \\
  Entry Exam & 2.430\tnote{***} & 1.021\tnote{*} & 1.725\tnote{***} \\
  & (0.589) & (0.574) & (0.551) \\
  Cognitive & -0.383 & -0.395 & -0.389 \\
  & (0.533) & (0.516) & (0.497) \\
  Social & 0.363 & 0.306 & 0.335 \\
  & (0.403) & (0.427) & (0.378) \\
  Constant & 12.379\tnote{***} & 11.188\tnote{***} & 11.783\tnote{***} \\
  & (4.306) & (3.910) & (3.814) \\
  \midrule
  Individual controls & Yes & Yes & Yes \\
  Observations & 633 & 633 & 633 \\
  R-squared & 0.157 & 0.102 & 0.141 \\
  \bottomrule
  \end{tabular*}
  \begin{tablenotes}[flushleft]
  \footnotesize
  \item[] \textit{Note:} Robust standard errors in parentheses. * $p < 0.1$, ** $p < 0.05$, *** $p < 0.01$. Dependent variables are the percentage of referrals received relative to potential referrals for cognitive (1), social (2), and all referrals (3). Controls include gender, age and semester. Sample is restricted to participants with complete administrative and experimental data.
  \end{tablenotes}
  \end{threeparttable}
  \end{table}

  \subsection{Social Class}
  In our main empirical specification, we document social class homophily and how it interacts with performance, when controlling classroom SES composition.\footnote{See \citet{mcpherson2001birds} for a discussion on homophily in general, for evidence on social class homophily see \citet{chetty2022social,chetty2022socialb}. For experimental evidence on a different type of homophily (gender) in referrals see \citet{beaman_job_2018}.} Our dependent variable is still the fraction of referrals received by total number of referrals, but now decomposed by the social class of referrers. We consider the share of referrals coming from high SES for relevance and readability, and present the equivalent analysis for low SES later. We estimate referral fraction $y_{i}$: 
  \begin{equation}
    y_{i} = \beta_0 + \beta_1{GPA}_{i}\times {SES}_{i} + \beta_2{E.Exam}_{i} + \beta_3{ClassroomSES}_{i}  + \mathbf{X}_{i}\boldsymbol{\gamma} + \epsilon_{i}    
  \end{equation}
  Our independent variables are participant social class and the fraction of low SES classmates, in addition to relevant performance indicators identified earlier. We include an interaction term between performance and social class to tease out the relationship between performance and social class. We consider interaction with GPA because there are no SES differences in this measure (see Appendix Figure \ref{fig:gpa}). Individual controls (age, gender, semester) are in $\mathbf{X}_{i}$.\footnote{The relationship between social class and referrals may operate through other channels than socioeconomic status, for example being a first-generation college student, having rural background, etc., which strongly correlate with low SES status. We focus on the total effect of social class through SES, and explore how this effect interacts with academic performance.}
...
\begin{table}
  ...
  \toprule
  & (1) & (2) & (3) \\
  Dep. Var. & All & Baseline & All \\
  \midrule
  GPA & 2.967\tnote{***} & 1.877\tnote{***} & 3.037\tnote{***} \\
  & (0.441) & (0.279) & (0.523) \\
  Entry Exam & 1.725\tnote{***} & 0.816\tnote{**} & 2.032\tnote{***} \\
  & (0.551) & (0.334) & (0.600) \\
  Cognitive & -0.389 & -0.093 & -0.431 \\
  & (0.497) & (0.286) & (0.532) \\
  Social & 0.335 & 0.032 & -0.017 \\
  & (0.378) & (0.221) & (0.348) \\
  Constant & 11.783\tnote{***} & 5.504\tnote{**} & 12.905\tnote{***} \\
  & (3.814) & (2.217) & (3.195) \\
  \midrule
  Classroom FE & No & No & Yes \\
  Individual Controls & Yes & Yes & Yes \\
  Observations & 633 & 633 & 633 \\
  R-squared & 0.141 & 0.130 & 0.153 \\
  \bottomrule
  \end{tabular*}
  \begin{tablenotes}[flushleft]
  \footnotesize
  \item[] \textit{Note:} Robust standard errors in parentheses. * $p < 0.1$, ** $p < 0.05$, *** $p < 0.01$. The dependent variable is the percentage of referrals received relative to potential referrals. Column (1) shows results for all referrals, column (2) for baseline condition only, and column (3) includes classroom fixed effects for all referrals. Controls include gender, age and semester. Sample is restricted to participants with complete administrative and experimental data.
  \end{tablenotes}
  \end{threeparttable}
  \end{table}
