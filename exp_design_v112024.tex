...
Our first goal is understanding which individuals get referred more often. We consider the fraction of referrals received by total number of referrals that could have been received as our dependent variable (see Appendix Figure \ref{fig:referral_fraction} for the distribution). Because every referrer nominates 3 classmates per skill, analyzing only the extensive margin, i.e., whether an individual gets a referral, is not very informative.\footnote{Only 86 of the 849 students (10\%) never get a referral for either skill.} Our approach combines the intensive and extensive margins and also makes comparisons across classrooms with different sizes easier.\footnote{The number of participating students in a classroom mechanically drives the number of total referrals that could be received by a classmate. By normalizing referrals we focus on differences within instead of between classrooms.} Our three dependent variables are fractions of: (i) cognitive skill referrals, (ii) social skill referrals, and (iii) all referrals.

Under performance pay, higher performing classmates in skill tests should collect more referrals. However, it is not clear whether skills can be observed by referrers.\footnote{A relevant reason for this could be that we did not provide skill feedback during the experiment. Feedback during sessions could have contaminated referral choices because participants would have incentive to share skill test results.} We therefore include a common academic performance indicator that may affect our dependent variables, namely the standardized GPA. We control for basic demographics (age, gender, semester) from Table \ref{tab:demographic-comparison} in $\mathbf{X}_{i}$ and exclude socioeconomic characteristics which will be examined in the next section. We estimate three referral fractions $y_{i}$: 
\begin{equation}\label{eq1}
    y_{i} = \beta_0 + \beta_1{GPA}_{i} + \beta_2{Cognitive}_{i} + \beta_3{Social}_{i} + \mathbf{X}_{i}\boldsymbol{\gamma} + \epsilon_{i}    
\end{equation}
Table \ref{tab:regression-results1} illustrates our first set of findings. Our comparison of interest is between skills and GPA. Conditional on GPA and individual correlates specified above, there is no statistically significant difference between skill level and the fraction of referrals received for that skill (columns 1-2). This holds true also when referrals for both skills are aggregated (column 3). We emphasize aggregated fraction of referrals because most referrals overlap across skills, as illustrated in Appendix Figure \ref{fig:same_}. Point estimates for skills are very close to zero. The 95\% confidence intervals rule out that a one standard deviation increase in either skill causes more than a 1 percentage point difference in the fraction of referrals received. This is an order of magnitude smaller margin when compared to the effects of a one standard deviation increase in GPA. 

A one standard deviation increase in GPA increases the fraction of referrals by about 3 percentage points on a base rate of 15\% across the three dependent variables. This is a modest 20-percent increase in the fraction of referrals received.\footnote{The average participant in the average classroom with about 21 students receives 2.3 referrals. A one standard deviation increase in GPA increases this number to 2.8 referrals.} Smaller point estimates for GPA and lower explained variance suggests our referral model performs better for cognitive skill. Social skill referrals are seemingly made with other considerations not entirely captured in the model. The results suggest participants are not referring classmates who score high on skill tests but refer instead based on their academic performance as measured by GPA.\footnote{Our findings are robust to restricting the observations to the baseline/quota treatments and including classroom fixed effects (see Appendix Table \ref{tab:regression-results-1-appendix1}).} This may be due to classmates being able to observe academic performance of peers but not their skill test results. To expand a bit on the implications of this finding, we show that there is little correlation between skills and GPA (see Appendix Table \ref{tab:correlation}). A strategy that consists of referring high-GPA students to identify high-skill peers seems to have backfired. This explains the lack of skill differences between the sample and referrals observed in our data.
...
\begin{table}[H]
  ...
  \midrule
  GPA & 3.303\tnote{***} & 2.759\tnote{***} & 3.031\tnote{***} \\
  & (0.463) & (0.452) & (0.419) \\
  Cognitive & 0.554 & 0.000 & 0.277 \\
  & (0.473) & (0.446) & (0.430) \\
  Social & 0.668\tnote{*} & 0.235 & 0.451 \\
  & (0.403) & (0.416) & (0.372) \\
  Constant & 14.977\tnote{***} & 14.042\tnote{***} & 14.509\tnote{***} \\
  & (2.252) & (2.614) & (2.238) \\
  \midrule
  Individual controls & Yes & Yes & Yes \\
  Observations & 665 & 665 & 665 \\
  R-squared & 0.105 & 0.094 & 0.111 \\
  \bottomrule
  \end{tabular*}
  \begin{tablenotes}[flushleft]
  \footnotesize
...
In this section we focus on various other specifications of our model in Equation \ref{eq1}. In Table \ref{tab:rav-rob1}, we consider cognitive skill and incrementally add independent variables. A one standard deviation increase in cognitive skill test score increases the fraction of cognitive skill referrals by about 1.1 points on a base rate of 13\% (column 1). This effect disappears when GPA is added in the model (column 2) and the 95\% confidence intervals rule out that a one standard deviation increase in cognitive skill causes more than a 1 percentage point difference in the fraction of referrals received. Our GPA estimate without controls causes a 3 percentage points increase on a base rate of 12.5\%, consistent with the specification in Table \ref{tab:regression-results1}. Finally, with the addition of individual controls (age, gender, semester) and social skill in column 3, we obtain the estimates in \ref{tab:regression-results1} for cognitive skill. Adding individual controls does not change our performance estimates in a meaningful way. 
\begin{table}[H]
  \centering
  \begin{threeparttable}
  \caption{Percentage of cognitive referrals conditional on skills}\label{tab:rav-rob1}
  \begin{tabular*}{0.85\textwidth}{@{\extracolsep{\fill}}l c c c@{}}
  \toprule
  & (1) & (2) & (3) \\
  Dep. Var. & Cognitive & Cognitive & Cognitive \\
  \midrule
  Cognitive & 1.111\tnote{***} & 0.677 & 0.554 \\
  & (0.427) & (0.433) & (0.473) \\
  GPA &  & 3.187\tnote{***} & 3.303\tnote{***} \\
  &  & (0.443) & (0.463) \\
  Social &  &  & 0.668\tnote{*} \\
  &  &  & (0.403) \\
  Constant & 12.671\tnote{***} & 12.483\tnote{***} & 14.977\tnote{***} \\
  & (0.404) & (0.399) & (2.252) \\
  \midrule
  Individual controls & No & No & Yes \\
  Observations & 713 & 668 & 665 \\
  R-squared & 0.010 & 0.091 & 0.105 \\
  \bottomrule
  \end{tabular*}
  \begin{tablenotes}[flushleft]
  \footnotesize
  \item[] \textit{Note:} Robust standard errors in parentheses. * $p < 0.1$, ** $p < 0.05$, *** $p < 0.01$. Dependent variable is the percentage of cognitive referrals received relative to potential referrals. Controls include gender, age and semester. Sample is restricted to participants with complete administrative and experimental data. All continuous variables are standardized.
  \end{tablenotes}
  \end{threeparttable}
\end{table}
In Table \ref{tab:eye-rob1}, we do the same exercise for social skill and incrementally add independent variables. A one standard deviation increase in social skill test score does not increase the fraction of social skill referrals (column 1). This null-effect persists when GPA is added in the model (column 2) and the 95\% confidence intervals rule out that a one standard deviation increase in social skill causes more than a 0.5 percentage point difference in the fraction of referrals received. Our GPA estimate without controls causes a 2.7 percentage points increase on a base rate of 12.6\%, consistent with the specification in Table \ref{tab:regression-results1}. Finally, with the addition of individual controls (age, gender, semester) and cognitive skill in column 3, we obtain the estimates in Table \ref{tab:regression-results1} for social skill. Adding individual controls does not change our performance estimates in a meaningful way, but increases the variance explained by the model when compared to cognitive skill. This is mostly due to female participants receiving significantly more referrals for social skill.    
\begin{table}[H]
  \centering
  \begin{threeparttable}
  \caption{Percentage of social referrals conditional on skills}\label{tab:eye-rob1}
  \begin{tabular*}{0.85\textwidth}{@{\extracolsep{\fill}}l c c c@{}}
  \toprule
  & (1) & (2) & (3) \\
  Dep. Var. & Social & Social & Social \\
  \midrule
  Social & 0.554 & 0.284 & 0.235 \\
  & (0.389) & (0.408) & (0.416) \\
  GPA &  & 2.773\tnote{***} & 2.759\tnote{***} \\
  &  & (0.428) & (0.452) \\
  Cognitive &  &  & 0.000 \\
  &  &  & (0.446) \\
  Constant & 12.785\tnote{***} & 12.587\tnote{***} & 14.042\tnote{***} \\
  & (0.403) & (0.396) & (2.614) \\
  \midrule
  Individual controls & No & No & Yes \\
  Observations & 713 & 668 & 665 \\
  R-squared & 0.003 & 0.068 & 0.094 \\
  \bottomrule
  \end{tabular*}
  \begin{tablenotes}[flushleft]
  \footnotesize
  \item[] \textit{Note:} Robust standard errors in parentheses. * $p < 0.1$, ** $p < 0.05$, *** $p < 0.01$. Dependent variable is the percentage of social referrals received relative to potential referrals. Controls include gender, age and semester. Sample is restricted to participants with complete administrative and experimental data. All continuous variables are standardized.
  \end{tablenotes}
  \end{threeparttable}
\end{table}
...
% Sensitivity analysis incrementally adding academic performance measures is in Appendix Table \ref{tab:regression-results-1-appendix1-cognitive} and Table \ref{tab:regression-results-1-appendix1-social}.
  ...
  \begin{table}
    \centering
    \begin{threeparttable}
    \caption{Percentage of referrals from high SES referrers}\label{tab:regression-results2}
    \begin{tabular*}{0.85\textwidth}{@{\extracolsep{\fill}}l c c c c@{}}
    \toprule
    & (1) & (2) & (3) & (4) \\
    Dep. Var. & High SES & High SES & High SES & High SES \\
    \midrule
    Low SES & -1.318\tnote{***} & -1.112\tnote{**} & -1.167\tnote{**} & -0.780\tnote{*} \\
    & (0.465) & (0.477) & (0.471) & (0.434) \\
    GPA &  & 1.035\tnote{***} & 0.253 & 0.351 \\
    &  & (0.256) & (0.411) & (0.365) \\
    Entry Exam &  & 0.894\tnote{***} & 0.948\tnote{***} & 0.749\tnote{***} \\
    &  & (0.277) & (0.275) & (0.254) \\
    Low SES × GPA &  &  & 1.404\tnote{***} & 1.042\tnote{**} \\
    &  &  & (0.499) & (0.450) \\
    Classroom \% Low SES &  &  &  & -0.149\tnote{***} \\
    &  &  &  & (0.020) \\
    Constant & 6.180\tnote{***} & 5.283\tnote{**} & 5.017\tnote{**} & 14.570\tnote{***} \\
    & (1.193) & (2.180) & (2.185) & (2.520) \\
    \midrule
    Baseline controls & Yes & Yes & Yes & Yes \\
    Observations & 666 & 633 & 633 & 633 \\
    R-squared & 0.015 & 0.076 & 0.090 & 0.200 \\
    \bottomrule
    \end{tabular*}
    \begin{tablenotes}[flushleft]
    \footnotesize
    \item[] \textit{Note:} Robust standard errors in parentheses. * $p < 0.1$, ** $p < 0.05$, *** $p < 0.01$. The dependent variable is the percentage of referrals received from high SES classmates. Controls include gender, age and semester. Sample is restricted to participants with complete administrative and experimental data.
    \end{tablenotes}
    \end{threeparttable}
    \end{table}
    ...
    \begin{table}
      \centering
      \begin{threeparttable}
      \caption{Percentage of referrals from low SES referrers}\label{tab:regression-results-lses}
      \begin{tabular*}{0.85\textwidth}{@{\extracolsep{\fill}}l c c c c@{}}
      \toprule
      & (1) & (2) & (3) & (4) \\
      Dep. Var. & Low SES & Low SES & Low SES & Low SES \\
      \midrule
      Low SES & 0.375 & 0.663 & 0.681 & 0.335 \\
      & (0.554) & (0.537) & (0.530) & (0.519) \\
      GPA &  & 1.931\tnote{***} & 2.191\tnote{***} & 2.104\tnote{***} \\
      &  & (0.319) & (0.500) & (0.482) \\
      Entry Exam &  & 0.757\tnote{**} & 0.739\tnote{**} & 0.917\tnote{***} \\
      &  & (0.318) & (0.320) & (0.313) \\
      Low SES × GPA &  &  & -0.466 & -0.143 \\
      &  &  & (0.594) & (0.570) \\
      Classroom \% Low SES &  &  &  & 0.133\tnote{***} \\
      &  &  &  & (0.022) \\
      Constant & 7.552\tnote{***} & 6.660\tnote{**} & 6.748\tnote{**} & -1.788 \\
      & (1.607) & (2.697) & (2.708) & (2.856) \\
      \midrule
      Baseline controls & Yes & Yes & Yes & Yes \\
      Observations & 666 & 633 & 633 & 633 \\
      R-squared & 0.029 & 0.111 & 0.112 & 0.172 \\
      \bottomrule
      \end{tabular*}
      \begin{tablenotes}[flushleft]
      \footnotesize
      \item[] \textit{Note:} Robust standard errors in parentheses. * $p < 0.1$, ** $p < 0.05$, *** $p < 0.01$. The dependent variable is the percentage of referrals received from low-SES classmates. Controls include gender, age and semester. Sample is restricted to participants with complete administrative and experimental data.
      \end{tablenotes}
      \end{threeparttable}
      \end{table} 
...
\begin{figure}[!htbp]
  ...
  \caption*{\footnotesize\textit{Note:} The figure shows the fraction of referrals received across cognitive and social skills, disaggregated by baseline and quota treatments. The fraction is calculated as the number of actual referrals received divided by the maximum possible referrals a student could have received. To illustrate: Consider a classroom with 25 students where 20 participated in the experiment, with 10 students in each treatment group. Each student could receive a maximum of 10 referrals (excluding self-referrals) from others in their treatment group. We normalize the total referrals received by this maximum possible number to obtain the fraction shown in the figure. The distribution of referrals is right-skewed with extreme outliers. While the median student across treatments receives 9 percent of possible referrals, those above the 90th percentile receive between 27 and 100 percent.}
\end{figure}
...
\begin{figure}[!htbp]
  ...
  \begin{tablenotes}
    \footnotesize
    \item[] {\textit{Note:} There are no significant differences in standardized GPA between high-SES (M = 0.058, SD = 1.032, n = 286) and low-SES students (M = 0.033, SD = 0.970, n = 382), $t(666)$ = 0.32, $p$ = 0.746, $d$ = 0.025. }
  \end{tablenotes}
\end{figure}
