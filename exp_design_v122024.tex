...
where $n_c$ represents the number of participants in the Baseline condition in classroom $c$. The indicator $r_{ijc}^s$ takes value 1 if participant $j$ in the Baseline condition refers individual $i$ for skill $s$, and 0 otherwise, requiring both $i$ and $j$ to be in the same classroom $c$. The denominator $n_c - \mathbbm{1}(i \in B_c)$ accounts for the maximum possible referrals that individual $i$ could receive. If $i$ is in the Baseline condition ($\mathbbm{1}(i \in B_c)=1$), we subtract one from $n_c$ to account for the self-referral restriction.\footnote{We find that 33.8 percent of participants in the sample for cognitive and social skills self-referred, while explicitly instructed not to do so. In Appendix Table \ref{tab:self_ref} we compare the outcomes of the subsample who self-refer. Those who self-refer are 10 percent more likely to be low-SES, and have systematically lower cognitive skills (0.2 SD) and GPA (0.25 SD). As self-referrerals are not informative and add noise to our estimates, we drop these instances from our paired referral-referrer sample in subsequent analyses. Self-referrering participants' remaining referral choices are kept in the dataset.}
...
Diversity in referral choices vary substantially between participants as they nominate 3 individuals per skill and can make overlapping referrals across skills. In Appendix Figures \ref{fig:same_a} and \ref{fig:same_b}, we show that only 20.5 percent of participants pick all unique classmates for their 6 choices. In fact, depending on our measures, between 50 to 70 percent of participants make 2 or 3 overlapping referrals and the diversity in referral choices is much lower than what we anticipated.\footnote{In Appendix Table \ref{tab:overlap_ref} we compare the characteristics of referrers make unique referrals to the remainder of the sample. The results suggest minimal differences in GPA and skills.}

With such a large share overlapping referral choices, participants could hold beliefs that classmates who with higher scores in one skill could also have higher scores in the other skill. Would such beliefs be accurate? There is only modest ($\rho = 0.267$) correlation between the two skills (see Appendix Table \ref{tab:correlation}). To understand whether making overlapping referrals is strategic, we turn to the incentives. Participants were incentivized to pick the top 3 performers in their classroom for each skill to earn a fixed bonus. Looking at the characteristics of top skilled participants in Appendix Table \ref{tab:top_perf}, we find that conditional on being among the top 3 for one of the skills, only 1 in 3 participants were in the top 3 for both skills. This suggests making more than 1 overlapping referral choice would decrease the chances to win the bonus. 

Coupled with the notable difficulties in identifying skills documented in the previous section, a competing explanation for the amount of overlapping referral choices would be that individuals who refer classmates twice for both skills are worse at observing skills. This would imply underlying heterogeneity in skill identification resulting in different referral strategies. We can test these competing hypotheses in our data: If ``twice'' referrers -defined as those who refer an individual for both skills- are better at identifying at least one of the skills, this would give more credence to beliefs about correlated skills. On the other hand, if ``twice'' referrers are worse in skill identification compared to single-referrers or use GPA proxy for referrals, it would imply that they have no information about skills. We compare peers who recieve referrals twice from their classmates to those who recieve referrals only once per skill. Formally, let indicator $r_{ijc}^{twice}$ take value 1 if individual $j$ referred individual $i$ for both skills. The percentage share of referrals received by individual $i$ from participants in classroom $c$ and in Baseline condition ($\forall j \in B_c$) is: 

\begin{equation}
  y_{ic}^{twice} = \frac{\sum_{j \neq i} r_{ijc}^{twice}}{n_c - \mathbbm{1}(i \in B_c)} \times 100
  \end{equation}

where $n_c$ represents the number of participants in the Baseline condition in classroom $c$. The indicator $r_{ijc}^{twice}$ takes value 0 if participant $j$ in the Baseline condition does not refer individual $i$ for both skills. The denominator $n_c - \mathbbm{1}(i \in B_c)$ accounts for the maximum possible ``twice'' referrals that individual $i$ could receive. If $i$ is in the Baseline condition ($\mathbbm{1}(i \in B_c)=1$), we subtract one from $n_c$ to account for the self-referral restriction as before. Similarly, let $r_{ijc}^{s,single}$ take value 1 if individual $j$ referred individual $i$ only for skill $s$. The percentage share of ``single'' referrals received by individual $i$ from participants in classroom $c$ and in Baseline condition ($\forall j \in B_c$) for skill $s$ is: 

\begin{equation}
  y_{ic}^{s,single} = \frac{\sum_{j \neq i} r_{ijc}^{s,single}}{n_c - \mathbbm{1}(i \in B_c)} \times 100
  \end{equation}
and it follows trivially that for any $s$, percentage share of ``single'' and ``twice'' referrals received by individual $i$ from participants in classroom $c$ and in Baseline condition ($\forall j \in B_c$) must add up to the total share of referrals received:
\begin{equation}
  y_{ic}^{s} = y_{ic}^{s,single} + y_{ic}^{twice}
  \end{equation}
we regress Equation \ref{skill} supplanted with GPA for our three new dependent variables and report our findings in \ref{tab:regression-results3}. 
...
\begin{table}[H]
  ...
  \begin{tablenotes}[flushleft]
  \footnotesize
  \item[] \textit{Note:} Classroom-level clusted standard errors in parentheses. * $p < 0.1$, ** $p < 0.05$, *** $p < 0.01$. The dependent variable in column (1) is the percentage of ``double'' referrals received from the same referrer, in column (2) for cognitive skill only, and in column (3) for social skill only. Cognitive and social skills are the respective standardized test scores. GPA is standardized to mean zero and unit variance.
  \end{tablenotes}
  \end{threeparttable}
\end{table}

 \subsection*{Social Class and Quota}
  ...
  \begin{table}[H]
    \centering
    \begin{threeparttable}
    \caption{Treatment effect on SES controlling for performance}\label{tab:treatment-effects}
    \begin{tabular*}{0.85\textwidth}{@{\extracolsep{\fill}}l c c@{}}
    \toprule
    & (1) & (2) \\
    & Cognitive & Social \\
    ...
    \begin{tablenotes}[flushleft]
    \footnotesize
    \item[] \textit{Note:} Classroom-level clusted standard errors in parentheses. * $p < 0.1$, ** $p < 0.05$, *** $p < 0.01$. The dependent variables are the percentage of referrals received for cognitive and social skills. Quota is a dummy for referrals received from classmates in the quota condition. Low-SES is a dummy for participant's socioeconomic status. Skill refers to standardized Raven's test score for cognitive referrals (column 1) and Reading the Mind in the Eyes test score for social referrals (column 2). GPA is standardized to mean zero and unit variance.
    \end{tablenotes}
    \end{threeparttable}
  \end{table}
...
\begin{figure}[H]
  ...
  \includegraphics[width=0.85\linewidth]{ref_skill_baseline.png}
...
\begin{figure}[H] \label{fig:same_combined}
  ...
  \begin{subfigure}[b]{0.48\linewidth}
      ...
      \includegraphics[width=\linewidth]{ses_dist.png}
      \label{fig:same_a}
  \end{subfigure}
  \hfill
  \begin{subfigure}[b]{0.48\linewidth}
      ...
      \includegraphics[width=\linewidth]{ses_dist3.png}
      \label{fig:same_b}
  \end{subfigure}
